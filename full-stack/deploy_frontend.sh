...
invalidation_id=$(echo "$invalidation_output" | grep -o '(?<="Id": ")[^"]+')
echo $invalidation_id

# Wait for cloudfront invalidation to complete
aws cloudfront wait invalidation-completed --distribution-id $cloudfront_distribution_id --invalidation-id $invalidation_id
